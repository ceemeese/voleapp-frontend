...
export const activateMemberAction = async (clubId: string, memberId: string) : Promise<void> => {
    try {
        const config = member.activateMember(clubId, memberId);
        ...
        if (status == 400) {
            ...
            throw new BusinessError('Ha habido un error en la activación del miembro. Intente de nuevo')
        }
