...
export const deactivateCourtAction = async (courtId: string) : Promise<void> => {
    try {
        const config = court.deactivateCourt(courtId);
        ...
        if (status == 400) {
            ...
            throw new BusinessError('Ha habido un error en la desactivación de la pista. Intente de nuevo')
        }
